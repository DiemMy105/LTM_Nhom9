...
namespace ChatTCP.Client.Forms
{
    public class ForwardDialog : Form
    {
        ...
        public string SelectedReceiverUsername { get; private set; } = string.Empty;
        ...
        private List<(string DisplayName, string Identifier, int? UserId, int? GroupId, bool IsGroup)> _items =
            new List<(string, string, int?, int?, bool)>();
        ...
        private void LoadTargets(List<User> users, List<Group> groups)
        {
            ...
            foreach (var group in groups)
            {
                ...
                _items.Add((group.GroupName, group.GroupName, null, group.GroupId, true));
            }
            ...
            foreach (var user in users)
            {
                ...
                lbTargets.Items.Add(display);
                _items.Add((user.DisplayName, user.Username, user.UserId, null, false));
            }
        ...
        private void BtnForward_Click(object? sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Vui lòng chọn người nhận hoặc nhóm muốn chuyển tiếp.",
            ...
            IsTargetGroup = item.IsGroup;

            if (item.IsGroup)
            {
                SelectedGroupId = item.GroupId;
                SelectedGroupName = item.Identifier;
            ...
            else
            {
                ...
                SelectedReceiverUsername = item.Identifier;
