    ...
    public class RegisterForm : Form
    {
            ...
            EventArgs e)
        {
            ...
            try
            {
                ...
                // Đọc ảnh vào MemoryStream
                // để không khóa file gốc.
        ...
        // hề được lưu lại hay gửi lên Server - nên sau khi RegisterForm đóng,
        // ảnh đã chọn biến mất hoàn toàn, Server luôn lưu Avatar mặc định.
        //
        // Giờ đây, lúc bấm "Đăng ký": nếu có avatar tùy chọn, resize nhỏ lại rồi
        // LƯU VẬT LÝ vào Resources/Avatars/{username}.png ngay trên máy Client
        // đang chạy (dùng chung thư mục với avt1.png/avt2.png có sẵn), rồi gửi
        // TÊN FILE đó lên Server để lưu vào DB (cột Avatar).
        //
        // GIỚI HẠN: chỉ hoạt động đúng khi Client đăng ký và Client đăng nhập là
        // CÙNG 1 MÁY (vì ảnh thật không được truyền qua mạng, chỉ truyền tên
        // file). Đăng ký ở máy khác rồi đăng nhập ở máy này sẽ không thấy đúng
        // avatar - đây là giới hạn kiến trúc hiện tại (chỉ truyền JSON/text qua
        // TCP, chưa có cơ chế truyền file ảnh giữa các máy).
        private string? SaveAvatarLocallyOrNull(string username)
        {
            if (string.IsNullOrWhiteSpace(_selectedAvatarPath))
            {
                // Không chọn avatar tùy chỉnh -> để Server tự dùng avatar mặc định.
                return null;
            }
            ...
            try
            {
                ...
                // Tên file theo username - username đã được validate trước đó
                // (không chứa khoảng trắng / ký tự '|'), nên an toàn làm tên file.
            ...
            {
                // Lỗi lưu avatar không nên chặn cả luồng đăng ký - chỉ log ra
                // status và tiếp tục đăng ký với avatar mặc định.
                ShowStatus(
            ...
            EventArgs e)
        {
            ...
            try
            {
                ...
                // ĐÃ SỬA: lưu avatar tùy chọn (nếu có) vào Resources/Avatars/ của
                // Client, rồi gửi tên file kèm theo lên Server qua RequestRegister.
